    ...
    <div className="flex items-center gap-2 flex-wrap">
      {FILTERS.map(({ label, value }) => (
        <button
          type="button"
          key={value}
          onClick={() => todoStore.dispatch.setFilter(value)}
      ...
      {doneCount > 0 && (
        <button
          type="button"
          onClick={() => todoStore.dispatch.clearDone()}
  ...
  return (
    ...
    >
      <input
        type="checkbox"
        checked={item.done}
        onChange={() => todoStore.dispatch.toggleTodo(item.id)}
      ...
      <button
        type="button"
        onClick={() => todoStore.dispatch.removeTodo(item.id)}
        aria-label="Remove"
  ...
  return (
    <div className="max-w-md mx-auto my-12 px-4">
      ...
      <p className="mt-6 text-center text-slate-400 text-sm">
        Kin Store example — better-redux · withPlugins · immer · persist
      </p>
    </div>
  );
}
