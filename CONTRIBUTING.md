...
up locally, the conventions the codebase follows, and how to submit changes.
...
3. Run `deno lint` and `deno task test` before opening a PR; the pre-commit
   hook runs the same checks and will block a commit that fails them.
...
- Commit subjects follow `type(scope): summary` (e.g. `fix(core): ...`,
  ...
  body.
- Version bumps and `CHANGELOG.md` entries are their own commits, made only
  when preparing to publish, never mixed into a `feat`/`fix`/`refactor` commit.

See [CLAUDE.md](./CLAUDE.md) for the full architecture and conventions
reference used when working on this codebase.

## Reporting bugs and requesting features

Use the issue templates when opening a
[new issue](https://github.com/jolleekin/kin-store/issues/new/choose).
For open-ended questions or design discussions, use
[Discussions](https://github.com/jolleekin/kin-store/discussions) instead.
