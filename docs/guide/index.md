...
1. **No type safety** — `state` inside the immer callback is `any`. TypeScript can't help you here.
2. **State and actions are indistinguishable** — `todos` (data) and `addTodo` (a command) live in the same object. You can't tell from the type alone what's state and what's behavior.
3. **Stable refs through subscriptions** — `addTodo` is stable and will never change, yet `useStore(s => s.addTodo)` registers a subscription that runs on every state update, forever.
4. **Inside-out middleware pipeline** — `devtools(persist(immer(...)))` reads left-to-right but executes right-to-left. You must mentally invert the nesting to understand execution order.
